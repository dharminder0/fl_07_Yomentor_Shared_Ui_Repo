...
const SkillsTestList = ({ route }: any) => {
  ...
  const [gradeId, setGradeId] = useState<any>(userInfo?.studentGradeId);
  ...
  const handleFilter = (type: any) => {
    if (type === 'clear') {
      ...
      setGradeId(userInfo?.studentGradeId);
    }
    setShouldFetch(true);
    ...
    <View>
      ...
      {skillList?.length > 0 ?
        <VirtualizedList
          ...
          style={{ height: height - (selectedSubject > 0 || complexityLevel > 0 || gradeId > 0 ? 140 : 115) }}
          windowSize={(selectedSubject > 0 || complexityLevel > 0 || gradeId > 0 ? 140 : 115)}
